...
function lineLabel() {
  ...
  var dx = onlyOneMerge ? 0 : 20;
  var totalWidth = 0;
  this.rootSelection$.select('.zLabel').selectAll('g.line-legend').each(function (d, i, g) {
    totalWidth += +d3.select(g[i]).select('rect').attr('width');
  }).data(this.data.line.slice().reverse()).join(function (enter) {
    return enter.append('g').attr('class', 'line-legend').attr('transform', function (d, i, g) {
      var group = d3.select(g[i]);
      var width = util.measureSvgText(d.label, _this.fontSize) + dx + 10;
      var isHide = _this.filter$.findIndex(function (f) {
        return f.key === d.key;
      }) !== -1;
      group.insert('rect').attr('x', -2).attr('y', -2).attr('width', width).attr('height', 20).attr('fill', 'transparent');

      if (!onlyOneMerge) {
        group.insert('path').attr('fill', 'none').attr('stroke', isHide ? '#aaaa' : d.color || 'inherit').attr('stroke-width', 4).attr('transform', "translate(0,".concat(-_this.fontSize / 2, "),scale(0.5)")) // .attr('d', 'M0 6 C4 0,8 0,12 6 S20 12,24 6') // 曲线
        .attr('d', 'M0 6,28 6');
      }

      group.insert('text').attr('dx', dx).attr('fill', isHide ? '#aaaa' : 'currentColor').attr('text-anchor', 'start').attr('stroke-width', 0.5).text(d.label);
      totalWidth = _this.scale.y ? -totalWidth - width : totalWidth + 10;

      if (onlyOneMerge) {
        totalWidth = (_this.width$ - width + 10) / 2;
      }

      return "translate(".concat(totalWidth, ",0)");
    });
  }, function (update) {
    return update.each(function (d, i, g) {
      var group = d3.select(g[i]);
      var isHide = _this.filter$.findIndex(function (f) {
        return f.key === d.key;
      }) !== -1;
      group.select('path').attr('stroke', isHide ? '#aaaa' : d.color || 'inherit');
      group.select('text').attr('fill', isHide ? '#aaaa' : 'currentColor').text(d.label);
    });
  });
...
function tipCompute(prevRes, point, scaleAxis) {
  ...
  if (crossX) {
    fData.forEach(function (_ref) {
      ...
      var yyKey = y2Scale && data.length > 0 && ok(data[0].y2) ? 'y2' : 'y';
    ...
    fData.forEach(function (_ref2) {
      ...
      var xxKey = x2Scale && data.length > 0 && ok(data[0].x2) ? 'x2' : 'x';
