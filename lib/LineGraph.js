...
function lineLabel() {
  ...
  this.rootSelection$.select('.zLabel').selectAll('g.line-legend').each(function (d, i, g) {
    ...
    return enter.append('g').attr('class', 'line-legend').attr('transform', function (d, i, g) {
      ...
      if (!onlyOneMerge) {
        group.insert('path').attr('fill', 'none').attr('stroke', isHide ? '#aaaa' : d.color || 'inherit').attr('stroke-width', 4).attr('transform', 'translate(0,6),scale(0.5)') // .attr('d', 'M0 6 C4 0,8 0,12 6 S20 12,24 6') // 曲线
        .attr('d', 'M0 6,28 6');
      }
