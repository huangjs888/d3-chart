...
function updateElement(size) {
  ...
  var svgDiv = this.rootSelection$.select('div.svg').style('width', "".concat(zw, "px")).style('height', "".concat(zh, "px")).style('top', "".concat(padding[0], "px")).style('left', "".concat(padding[3] + 1, "px"));
  ...
  if (this.zoom.x || this.zoom.y) {
    svgDiv.select('.zReset').style('top', "".concat(-padding[0] - 3, "px")).style('left', "".concat(this.scale.y ? zw - offset - 18 : offset, "px"));
    offset += 23;
  }

  if (this.download) {
    svgDiv.select('.zDownload').style('top', "".concat(-padding[0] - 3, "px")).style('left', "".concat(this.scale.y ? zw - offset - 18 : offset, "px"));
    offset += 23;
  }
...
function createElement(container, size) {
  ...
  var divSelection = rootSelection.append('div').attr('class', 'svg').style('position', 'absolute').style('top', 0).style('left', 0);
  ...
  ['x', 'y', 'z'].forEach(function (key) {
    if (_this6.zoom[key] || key === 'z' && (_this6.zoom.x || _this6.zoom.y)) {
      divSelection.append('div').attr('class', key === 'z' ? 'zReset' : "".concat(key, "Lock")).style('position', 'absolute').attr('title', key === 'z' ? '重置视图' : "".concat(key.substr(0, 1).toUpperCase(), " \u8F74\u7F29\u653E\u5F00\u5173")).style('font-size', '18px').append('svg').attr('xmlns', svgXmlns).attr('fill', 'currentColor').attr('viewBox', '0 0 1024 1024').attr('width', '1em').attr('height', '1em').append('path').attr('d', key === 'z' ? resetPath : unlockPath);
    }
  });

  if (this.download) {
    divSelection.append('div').attr('class', 'zDownload').style('position', 'absolute').attr('title', '下载视图').style('font-size', '18px').append('svg').attr('xmlns', svgXmlns).attr('fill', 'currentColor').attr('viewBox', '0 0 1024 1024').attr('width', '1em').attr('height', '1em').append('path').attr('d', downloadPath);
  }
...
function bindEvents() {
  ...
  if (this.zoom.x || this.zoom.y) {
    this.rootSelection$.select('.zReset').on('click', function () {
      _this7.reset$();
  ...
  if (this.download) {
    this.rootSelection$.select('.zDownload').on('click', function () {
