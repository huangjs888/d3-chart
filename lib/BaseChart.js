 ...
 * @LastEditors: Huangjs
 * @LastEditTime: 2021-10-15 11:02:02
 * @Description: ******
 */
...
function updateElement(size) {
  ...
  if (this.scale.x) {
    ...
    group
      .select('.xLabel')
      .attr('transform', `translate(${zw / 2} ${zh + padding[2] - 14}) rotate(0)`);
  }
  if (this.scale.x2) {
  ...
  if (this.scale.y2) {
    ...
    group
      .select('.y2Label')
      .attr('transform', `translate(${zw + padding[1] - 14} ${zh / 2}) rotate(-90)`);
  }
  ...
  group
    ...
    .attr(
      'transform',
      `translate(${this.scale.y ? zw - offset : offset} ${this.scale.x2 ? 0 : -padding[0]})`,
    );
  this.width = width;
  this.height = height;
...
function createElement(container, size) {
  const pathClipId = `clip-${util.uuID()}`;
  ...
  this.zoomSelection$
    ...
    .on('mouseout', (e) => {
      ...
      if (this.tooptip) {
        ...
        if (
          !toElement ||
          (toElement.parentNode !== zNode && toElement.parentNode.parentNode !== zNode)
        ) {
