...
function updateZoom() {
  ...
  var viewExtent = [[0, 0], // @ts-ignore
  [this.width$, this.height$]];
  var scaleExtent = [[1, 1], [1, 1]];
  var translateExtent = [[0, 0], [0, 0]]; // @ts-ignore
  ...
  if (xzoom || yzoom) {
    ...
    var _ref13 = yzoom.precision || [yDomainEx / 10, yDomainEx * 10],
        ...
        _ref14$2 = _ref14[1],
        yMaxPrecision = _ref14$2 === void 0 ? yDomainEx * 10 : _ref14$2; // @ts-ignore


    var xRate = this.width$ / xDomainEx; // @ts-ignore
...
function updateElement(size) {
  ...
  var _computeSize = computeSize(this.rootSelection$.node(), size),
      ...
      rHeight = _computeSize.rHeight; // @ts-ignore


  var zw = width - rWidth - padding[3] - padding[1]; // @ts-ignore

  var zh = height - rHeight - padding[0] - padding[2];
  if (zw < 1) zw = 1;
  if (zh < 1) zh = 1; // @ts-ignore

  var group = this.rootSelection$.select('svg').attr('width', width).attr('height', height).select('g.group').attr('transform', "translate(".concat(padding[3], ",").concat(padding[0], ")")); // @ts-ignore

  this.rootSelection$.select('clipPath').select('rect').attr('width', zw).attr('height', zh); // @ts-ignore

  var baselineDelt = this.fontSize + 2; // @ts-ignore

  if (this.scale.x) {
    group.select('.xAxis').attr('transform', "translate(0,".concat(zh, ")")); // @ts-ignore

    if (this.scale.x.showRange || typeof this.scale.x.showRange === 'undefined') {
      group.select('.xLabel') // @ts-ignore
      .attr('transform', "translate(".concat(zw / 2, " ").concat(zh + padding[2] - iconSize + baselineDelt, ") rotate(0)"));
    }
  } // @ts-ignore


  if (this.scale.x2 && (this.scale.x2.showRange || typeof this.scale.x2.showRange === 'undefined')) {
    group.select('.x2Label') // @ts-ignore
    .attr('transform', "translate(".concat(zw / 2, " ").concat(baselineDelt - padding[0], ") rotate(0)"));
  } // @ts-ignore


  if (this.scale.y && (this.scale.y.showRange || typeof this.scale.y.showRange === 'undefined')) {
    group.select('.yLabel') // @ts-ignore
    .attr('transform', "translate(".concat(baselineDelt - padding[3], " ").concat(zh / 2, ") rotate(-90)"));
  } // @ts-ignore


  if (this.scale.y2) {
    group.select('.y2Axis').attr('transform', "translate(".concat(zw, ",0)")); // @ts-ignore

    if (this.scale.y2.showRange || typeof this.scale.y2.showRange === 'undefined') {
      group.select('.y2Label') // @ts-ignore
      .attr('transform', "translate(".concat(zw + padding[1] - iconSize + baselineDelt, " ").concat(zh / 2, ") rotate(-90)"));
    }
  } // @ts-ignore


  var svgDiv = this.rootSelection$.select('div.actions').style('width', "".concat(zw, "px")).style('height', "".concat(zh, "px")).style('top', "".concat(padding[0], "px")).style('left', "".concat(padding[3], "px")); // @ts-ignore

  if (this.zoom.x) {
    svgDiv.select('.xlock') // @ts-ignore
    .style('top', "".concat(this.scale.x ? zh + padding[2] - iconSize : -padding[0], "px")) // @ts-ignore
    .style('left', "".concat(this.scale.y ? zw - iconSize / 2 : -iconSize / 2, "px"));
  } // @ts-ignore


  if (this.zoom.y) {
    svgDiv.select('.ylock') // @ts-ignore
    .style('top', "".concat(this.scale.x ? -iconSize / 2 : zh - iconSize / 2, "px")) // @ts-ignore
    .style('left', "".concat(this.scale.y ? -padding[3] : zw + padding[1] - iconSize, "px"));
  }

  var offset = 5; // @ts-ignore

  if (this.zoom.x || this.zoom.y) {
    svgDiv.select('.reset') // @ts-ignore
    .style('top', "".concat(this.scale.x ? -padding[0] : zh + padding[2] - iconSize, "px")) // @ts-ignore
    .style('left', "".concat(this.scale.y ? zw - offset - iconSize : offset, "px"));
    offset += 23;
  } // @ts-ignore


  if (this.download) {
    svgDiv.select('.download') // @ts-ignore
    .style('top', "".concat(this.scale.x ? -padding[0] : zh + padding[2] - iconSize, "px")) // @ts-ignore
    .style('left', "".concat(this.scale.y ? zw - offset - iconSize : offset, "px"));
    offset += 23;
  }

  group.select('.zLabel') // @ts-ignore
  .attr('transform', // @ts-ignore
  "translate(".concat(this.scale.y ? zw - offset : offset, " ").concat( // @ts-ignore
...
function createElement(container, size) {
  ...
  .attr('fill', 'none').attr('text-anchor', 'middle') // @ts-ignore
  .attr('font-size', this.fontSize).attr('stroke', 'none').attr('stroke-width', 1).attr('stroke-linejoin', 'round').attr('stroke-linecap', 'round').style('position', 'absolute');
  ...
  axisType.forEach(function (key) {
    // @ts-ignore
    if (_this6.scale[key]) {
      groupSelection.append('g').attr('class', "".concat(key, "Axis")); // @ts-ignore

      if (_this6.scale[key].showRange || typeof _this6.scale[key].showRange === 'undefined') {
  ...
  actions.forEach(function (action) {
    divSelection.append('div').attr('class', action.className).style('position', 'absolute').attr('title', action.label).style('display', 'inline-flex').style('font-size', "".concat(iconSize, "px")).append('svg').attr('xmlns', svgXmlns).attr('fill', 'currentColor').attr('viewBox', '0 0 1024 1024').attr('width', '1em').attr('height', '1em').append('path').attr('d', action.path);
  }); // @ts-ignore

  if (this.tooptip) {
    // @ts-ignore
    var cross = this.tooptip.cross;
    zoomSelection.append('div').attr('class', 'tooptip').style('display', 'none').style('z-index', '999').style('border-radius', '8px').style('transition', 'none').style('padding', '10px 14px') // @ts-ignore
    .style('font-size', "".concat(this.fontSize, "px")).style('line-height', '22px').style('position', 'absolute').style('top', 0).style('left', 0);
  ...
  if (!(util.isNumber(size.width) && size.width > 1 && util.isNumber(size.height) && size.height > 1)) {
    var resize = util.debounce(function (e) {
      ...
      updateZoom.call(_this6); // @ts-ignore
      ...
      _this6.resize$.call(null, {
        ...
        // @ts-ignore
        transform: [d3.zoomTransform(_this6.zoomSelection$.node())].concat((0, _toConsumableArray2.default)(_this6.zoomSelection$.datum())),
...
function bindEvents() {
  ...
  var pointY0 = [0, 0];
  var point0 = [0, 0];
  var transform0 = d3.zoomIdentity; // @ts-ignore

  this.zoomSelection$.datum([d3.zoomIdentity, d3.zoomIdentity]); // @ts-ignore

  this.zoomer$.on('start', function (e) {
    // @ts-ignore
    if (_this7.destroyed || !_this7.rendered) return; // @ts-ignore
    ...
    if (sourceEvent.type !== 'call') {
      if (!_this7.xCanZoom$ && !_this7.yCanZoom$) return; // @ts-ignore
    ...
    if (sourceEvent.type !== 'call') {
      ...
      var point = d3.pointer(e, _this7.zoomSelection$.node()); // @ts-ignore
      ...
      if (Array.isArray(sourceEvent.transform)) {
        ...
        if (!_this7.xCanZoom$) xtf = sourceEvent.transform;
        if (!_this7.yCanZoom$) ytf = sourceEvent.transform;
      }
    ...
    Object.keys(scaleAxis).forEach(function (key) {
      var _scaleAxis$key = scaleAxis[key],
          ...
          scale = _scaleAxis$key.scale; // @ts-ignore

      _this7.rootSelection$.select(".".concat(key, "Axis")).call(axis); // @ts-ignore
    ...
    _this7.zoomend$.call(null, _objectSpread(_objectSpread({}, e), {}, {
      // @ts-ignore
      transform: [transform].concat((0, _toConsumableArray2.default)(_this7.zoomSelection$.datum())),
      scaleAxis: getScaleAxis.call(_this7)
    }));
  }); // @ts-ignore

  this.zoomSelection$ // @ts-ignore
  .call(this.zoomer$).on('mouseover', function (e) {
    // @ts-ignore
    if (_this7.destroyed) return; // @ts-ignore
    ...
    if (_this7.tooptip) {
      // @ts-ignore
      var cross = _this7.tooptip.cross; // @ts-ignore
    ...
    var scaleAxis = getScaleAxis.call(_this7); // @ts-ignore
    ...
    if (_this7.tooptip) {
      ...
      if (crossX || crossY) {
        ...
        if (display !== 'none') {
          ...
          var left = x0 + 10;
          var top = y0 - height - 10; // @ts-ignore
  ...
  if (this.zoom.x) {
    ...
    xlock.on('click', function () {
      ...
      _this7.setCanZoom('x', !_this7.xCanZoom$); // @ts-ignore


      _this7.canZoom$('x', _this7.xCanZoom$);
    });
  } // @ts-ignore
  ...
  if (this.zoom.y) {
    ...
    ylock.on('click', function () {
      ...
      _this7.setCanZoom('y', !_this7.yCanZoom$); // @ts-ignore


      _this7.canZoom$('y', _this7.yCanZoom$);
    });
  } // @ts-ignore
    ...
    this.rootSelection$.select('.download').on('click', function () {
      ...
      if (_this7.download.action) {
        ...
        _this7.download.action(function () {
          // @ts-ignore
          _this7.downloadImage(); // @ts-ignore
...
var BaseChart = /*#__PURE__*/function () {
  ...
  (0, _createClass2.default)(BaseChart, [{
    ...
    value: function destroy() {
      ...
      this.rendered = false;
      this.data = {};
      this.zoomer$.on('start', null).on('zoom', null).on('end', null); // @ts-ignore

      this.zoomSelection$.on('mouseover', null).on('mouseout', null).on('mousemove', null).on('dblclick.zoom', null).on('click', null).on('contextmenu', null); // @ts-ignore
    ...
    value: function resize() {
      if (document.createEvent) {
        ...
        window.dispatchEvent(event); // @ts-ignore
      } else if (document.createEventObject) {
        // @ts-ignore
        var _event = document.createEventObject();

        _event.type = 'onresize'; // @ts-ignore
    ...
    value: function downloadImage(name, content) {
      ...
      var tempCxt = tempCanvas.getContext('2d'); // @ts-ignore
