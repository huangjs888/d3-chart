    ...
    if (this.colorBar) {
      ...
      colorBar.selectAll('g').data(range).join(function (enter) {
        var tick = enter.append('g').attr('class', 'tick').attr('transform', function (_, i) {
          return "translate(24,".concat(i * height / length, ")");
        });
...
var HeatMap = /*#__PURE__*/function (_BaseChart) {
  ...
  function HeatMap() {
    ...
    if (_this.colorBar) {
      ...
      _this.rootSelection$.select('svg').select('.group').append('g').attr('class', 'heatColorBar').style('display', colorBar.show ? 'block' : 'none').attr('fill', 'currentColor').attr('transform', "translate(".concat(_this.width$ + _this.padding[1] - (_this.colorBar.width || 0), ",0)")).append('rect').attr('x', '0').attr('y', '0').attr('width', 24).attr('height', _this.height$).attr('fill', "url(#".concat(gradientId, ")"));
    }
