...
var _excluded = ["data", "tooptip", "padding", "colorBar", "scale"],
    _excluded2 = ["heat"],
...
function updateScale() {
  if (this.scale.z) {
    ...
    if (this.colorBar) {
      var gradient = this.rootSelection$.select('linearGradient');
      var colorBar = this.rootSelection$.select('.heatColorBar');
      var height = this.height$;
      var length = range.length - 1;
      var width = this.colorBar.width || 0;
      gradient.selectAll('stop').data(range).join('stop').attr('offset', function (_, i) {
        return "".concat(i * 100 / length, "%");
      }).attr('stop-color', function (c) {
        return c;
      });
      colorBar.selectAll('g').data(range).join(function (enter) {
        var tick = enter.append('g').attr('class', 'tick').attr('transform', function (_, i) {
          return "translate(".concat(width, ",").concat(i * height / length, ")");
        });
        tick.append('path').attr('d', 'M0,0 L4,4 L4,-4 Z');
        tick.append('text').attr('x', 8).attr('dy', '0.32em').attr('text-anchor', 'start').text(function (_, i) {
          return zFormat(domain[length - i]);
        });
        return tick;
      }, function (update) {
        var tick = update.attr('transform', function (_, i) {
          return "translate(".concat(width, ",").concat(i * height / length, ")");
        });
        tick.select('text').text(function (_, i) {
          return zFormat(domain[length - i]);
        });
        return tick;
      });
...
function doubleClick(point, _ref10, _ref11) {
  ...
  if (this.showHeat$) {
    ...
    if (selectX) {
      // 竖线
      var xval = +xScale.invert(x0);
      var xi = util.findNearIndex(xval, data.x, !average);
    ...
    if (selectY) {
      // 横线
      var yval = +yScale.invert(y0);
      var yi = util.findNearIndex(yval, data.y, !average);
...
var HeatMap = /*#__PURE__*/function (_BaseChart) {
  ...
  function HeatMap() {
    ...
    if (colorBar && colorBar.show) {
      var width = (colorBar.left || 0) + (colorBar.right || 0) + (colorBar.width || 0);
      var pad1 = !util.isNumber(pad[1]) || pad[1] < 0 ? 0 : pad[1];

      if (!pad.length) {
        pad = [0, width, 0, 0];
      } else if (pad.length === 1) {
        var pad0 = !util.isNumber(pad[0]) || pad[0] < 0 ? 0 : pad[0];
    ...
    if (_this.colorBar) {
      ...
      _this.rootSelection$.select('svg').select('.group').append('g').attr('class', 'heatColorBar').style('display', _this.colorBar.show ? 'block' : 'none').attr('fill', 'currentColor').attr('transform', "translate(".concat(_this.width$ + _this.padding[1] + 14 - (_this.colorBar.show ? (colorBar.left || 0) + (colorBar.right || 0) + (colorBar.width || 0) : 0), ",0)")).append('rect').attr('x', 0).attr('y', 0).attr('width', _this.colorBar.width || 0).attr('height', _this.height$).attr('fill', "url(#".concat(gradientId, ")"));
    }
  ...
  (0, _createClass2.default)(HeatMap, [{
    ...
    value: function setData(data, render, computeDomain) {
      ...
      var lineMarkX = this.zoomSelection$.select('.x-linemark');
