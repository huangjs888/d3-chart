...
function updateScale() {
  if (this.scale.z) {
    ...
    if (this.colorBar) {
      ...
      linearGradient.selectAll('stop').data(range).join('stop').attr('offset', function (_, i) {
        return "".concat(i * 100 / length, "%");
      }).attr('stop-color', function (c) {
        return c;
      });
      heatColorBar.selectAll('g').data(range).join(function (enter) {
  ...
  function HeatMap() {
    ...
    _this.resize$ = function (e, _ref15) {
      ...
      if (colorBar) {
        ...
        var length = ticks.size() - 1;
        ticks.attr('transform', function (_, i) {
          return "translate(".concat(colorBar.width, ",").concat(i * height / length, ")");
        });
      }
    };
