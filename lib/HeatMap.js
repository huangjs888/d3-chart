 ...
 * @LastEditors: Huangjs
 * @LastEditTime: 2021-10-15 11:02:17
 * @Description: ******
 */
...
function drawing(zContext, { xScale, yScale }, lineMark) {
  if (this.showHeat$) {
    ...
    lineMark
      .style('left', `${xp}px`)
      .style('visibility', xp < 0 || xp > this.width$ ? 'hidden' : 'visible');
...
class HeatMap extends BaseChart {
  constructor(...params) {
    ...
    const heatLabel = this.rootSelection$
      ...
      .attr(
        ...
        `translate(${this.scale.y ? 10 : this.width$ - 10},${
          this.scale.x2 ? 0 : -this.padding[0]
        })`,
      );
    heatLabel
      .append('text')
    ...
    this.resize$ = (e, { width, height, padding, ...rest }, ...args) => {
      ...
      heatLabel.attr(
        'transform',
        `translate(${this.scale.y ? 10 : width - 10},${this.scale.x2 ? 0 : -padding[0]})`,
      );
    };
    heatLabel.on('click', () => {
  ...
  destroy() {
    if (this.debounceDrawend$) this.debounceDrawend$.cancel();
