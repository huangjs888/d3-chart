...
function drawing(zContext, _ref5, _ref6) {
  ...
  if (this.showHeat$) {
    if (lineMarkX.node()) {
      var xp = xScale(lineMarkX.datum()) || 0;
      lineMarkX.style('left', "".concat(xp, "px")).style('visibility', xp < 0 || xp > this.width$ ? 'hidden' : 'visible');
    }

    if (lineMarkY.node()) {
      var yp = yScale(lineMarkY.datum()) || 0;
      lineMarkY.style('top', "".concat(yp, "px")).style('visibility', yp < 0 || yp > this.height$ ? 'hidden' : 'visible');
    }
  ...
  function HeatMap() {
    ...
    if (_this.tooptip) {
      ...
      select.split('').forEach(function (key) {
        if (key) {
          _this.zoomSelection$.append('div').attr('class', "".concat(key, "-linemark")).style('background', '#fa9305').style('display', 'none').style('position', 'absolute').style('top', 0).style('left', 0).style('width', key === 'x' ? '1px' : '100%').style('height', key === 'x' ? '100%' : '1px');
        }
      });
    }
