...
var matrixInterp = function matrixInterp(_ref, _ref2, c) {
  ...
  var _loop = function _loop(j) {
    ...
    var val1 = z[yInterp.bin1] || []; // 获取y轴(横向)范围内无效索引集合

    var yInvalidIndex = y.invalid || []; // 判断当前横向一整条像素的上bin0下bin1都是无效索引，则无效
    ...
    for (var i = x0; i < x1; i += 1) {
      ...
      if (!yInvalid) {
        (function () {
          ...
          var xInvalidIndex = x.invalid || []; // 判断当前像素点的左bin0右bin1都是无效索引，则无效
...
function tipCompute(prevRes, point, scaleAxis) {
  ...
  var _ref5 = cross === 'xy' ? this.getPointData({
    point: point,
    scaleAxis: scaleAxis
  }, average) : {
    point: point,
  ...
  if (value.length > 0) {
    ...
    var ndata = [].concat((0, _toConsumableArray2.default)(value.map(function (v, i) {
      var k = i === 0 ? 'x' : i === 1 ? 'y' : 'z';
      return _objectSpread(_objectSpread({}, scaleOpt[scaleAxis[k] ? k : "".concat(k).concat(k != 'z' ? '2' : '')]), {}, {
        value: v
      });
    })), (0, _toConsumableArray2.default)(prevValue));
    ...
    var result = function result(selection) {
      selection.selectAll('div').data(ndata).join('div').attr('style', 'white-space: nowrap;').html(function (d, i) {
        return i === 0 ? "".concat(d.label ? "".concat(d.label, ": ") : '').concat(d.format(d.value)).concat(d.unit || '') : "".concat(!d.color ? '' : "<span style=\"background: ".concat(d.color, "; width: 8px; height: 8px; border-radius: 50%; display: inline-block; margin-right: 8px;\"></span>"), "<span>").concat(d.label ? "".concat(d.label, ": ") : '', "</span><span style=\"display: inline-block; ").concat(!d.color ? '' : 'margin-left: 30px;', "\">").concat(d.format(d.value)).concat(d.unit || '', "</span>");
      });
    };
...
function doubleClick(point, _ref8) {
  ...
  var HeatMap = /*#__PURE__*/function (_BaseChart) {
    ...
    function HeatMap() {
      ...
      var _ref10 = (arguments.length <= 0 ? undefined : arguments[0]) || {},
          ...
          scale = _ref10.scale,
          restOptions = (0, _objectWithoutProperties2.default)(_ref10, _excluded);

      var _ref11 = data || {},
          heat = _ref11.heat,
          restData = (0, _objectWithoutProperties2.default)(_ref11, _excluded2);
      ...
      _this = _super.call(this, _objectSpread(_objectSpread({}, restOptions), {}, {
        ...
        tooltip: !tooltip ? false : _objectSpread(_objectSpread({
          cross: 'xy',
          select: '',
          average: true
        }, tooltip), {}, {
          compute: function compute(res) {
            ...
            if (typeof tooltip.compute === 'function') {
              ...
              result = (_tooltip$compute = tooltip.compute).call.apply(_tooltip$compute, [(0, _assertThisInitialized2.default)(_this), result].concat(args));
            }
      ...
      if (legend) {
        ...
        _this.rootSelection$.select('svg').select('.group').append('g').attr('class', 'heatLegend').style('display', legend.show ? 'block' : 'none').attr('fill', 'currentColor').attr('transform', "translate(".concat(_this.width$ + _this.padding[1] + legend.left, ",0)")).append('rect').attr('x', 0).attr('y', 0).attr('width', legend.width).attr('height', _this.height$).attr('fill', "url(#".concat(gradientId, ")"));
      }
      ...
      _this.resize$ = function (e, _ref12) {
        ...
        if (legend) {
          ...
          heatLegend.attr('transform', "translate(".concat(width + padding[1] + legend.left, ",0)"));
          ...
          ticks.attr('transform', function (v) {
            return "translate(".concat(legend.width, ",").concat(height * (1 - (v - startDomain) / deltDomain), ")");
          });
        }
      };
    ...
    (0, _createClass2.default)(HeatMap, [{
      key: "getPointData",
      value: function getPointData(e, avg) {
        var data = this.data.heat;
        var point = e.point,
            sourceEvent = e.sourceEvent,
            scaleAxis = e.scaleAxis;
        ...
        if (!currentPoint) {
          if (!sourceEvent) {
            ...
            var touches = sourceEvent.type === 'touchend' ? sourceEvent.changedTouches : sourceEvent.touches;
            currentPoint = d3.pointer(touches ? touches[0] : sourceEvent, this.zoomSelection$.node());
          }
        }

        var _currentPoint = currentPoint,
            _currentPoint2 = (0, _slicedToArray2.default)(_currentPoint, 2),
            x0 = _currentPoint2[0],
            y0 = _currentPoint2[1];
        ...
        if (this.showHeat$) {
          ...
          var _util$findNearIndex3 = util.findNearIndex(+yval, data.y),
              ...
              yi1 = _util$findNearIndex4[1]; // 确保point在图层内
      ...
      value: function setLineMark(lm, cb) {
        ...
        if (lineMarkY.node()) {
          ...
          var _zval = []; // yval在数据范围之内才可计算出zval
      ...
      value: function setData(data, render, computeDomain) {
        ...
        var _ref13 = data || {},
            heat = _ref13.heat,
            restData = (0, _objectWithoutProperties2.default)(_ref13, _excluded4);
